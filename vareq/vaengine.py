...
class AugmentedChat:
    ...
    def get_relevant_documents(self, query : str) -> List[str]:
        documents = self.lib.get_relevant_documents(query, self.config.max_knowledge_items)
        if len(documents) == 0:
        ...
        for document in documents:
            ...
            if total_size >= self.config.max_knowledge_size or total_count >= self.config.max_knowledge_items:
                break
