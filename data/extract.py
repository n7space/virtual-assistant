...
def extract_all_text_from_pdf(pdf_path: str, start_page : int, end_page : int) -> List[str]:
    all_lines = []
    try:
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Extract data from the input ECSS PDF file to the output text file.')
    ...
    parser.add_argument('--preamble', '-p', required=True, help='Preamble to start the output file with')
    parser.add_argument('--start-page', '-s', type=int, default=1, help='Page number to start extraction from (default: 1)')
    parser.add_argument('--end-page', '-e', type=int, default=1, help='Page number to end extraction at (default: 1)')
